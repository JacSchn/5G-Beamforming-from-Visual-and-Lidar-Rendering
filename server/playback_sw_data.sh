...
file_cnt=`ls ~/server_data/sweep_dump_data/ | wc -l`
echo $file_cnt
file_cnt=`expr ${file_cnt} - 1`

init_num=`cat ~/server_data/sweep_dump_data/0_sw_d | awk '{print $1}'`
...
do
	data_amt=`cat ~/server_data/sweep_dump_data/"${i}_sw_d" | awk '{print $1}'`
