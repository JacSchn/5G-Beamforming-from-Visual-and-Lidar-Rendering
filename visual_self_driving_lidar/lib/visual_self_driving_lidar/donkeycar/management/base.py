...
class CalibrateCar(BaseCommand):
    ...
    def parse_args(self, args):
        ...
        parser.add_argument('--channel', help='The channel youd like to calibrate [1-8]')
        parser.add_argument('--motorPWM', help='The motorPWM is [0-199]')
        parsed_args = parser.parse_args(args)
        return parsed_args

    def run(self, args):
        from donkeycar.parts.actuator import PCA9685
        from donkeycar.parts.actuator import motorPWM
        args = self.parse_args(args)
        if args.channel:
            #print("channel")
            ...
            while True:
                try:
                    val = input("""Enter a PWM (0-180)setting to test ('q' for quit),('savePWM'set dead ahead): """)
                    if val == 'savePWM':
                        c.savePWM()
                        print(">>>>>set dead ahead success! the default PWM is %d"%pmw)
                        break
                    if val == 'q' or val == 'Q':
                        break
                    pmw = int(val)
            ...
            while True:
                try:
                    val = input("""Enter a motorPWM (0-199)setting to test ('q' for quit): """)
                    if val == 'q' or val == 'Q':
                        break
                    pmw = int(val)
                    c.run(pmw)
